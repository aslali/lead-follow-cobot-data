...
ques = ['ad111', 'ad211', 'ad311']
all_ranks = [1, 2, 3, 4]
all_patterns = [1, 2, 3, 4]


def performance_accuracy(df):
    performance_accuracy_by_task(df)
    performance_accuracy_by_difficulty(df)
    performance_accuracy_by_pattern(df)


def performance_accuracy_by_task(df):
    ...
    sns.boxplot(y='value', x='Task', data=n1n)
...
def performance_accuracy_by_difficulty(df):
    var_name = 'Performance_accuracy'
    rtasks = []
    variable = []
...
def performance_accuracy_by_pattern(df):
    var_name = 'Performance_accuracy'
    pattern = []
    variable = []
    for i, row in df.iterrows():
